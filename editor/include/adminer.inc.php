...
<?php
	...
	# Table & slug() function below come from lib/web.php in the F3 project
	#	by Bong Cosca (& Christian Knuth) http://fatfreeframework.com
	function diacritics() {
		return [
	...
	function slug($text) {
		return trim(preg_replace('/([^\pL\pN])+/u','-', trim(strtr($text,$this->diacritics()))),'-');
	}
	...
	function tablesPrint($tables) {
		...
		$o = $this;
		usort($tables, function($a,$b) use ($o) {
			$an = $o->slug($o->tableName($a));
			$bn = $o->slug($o->tableName($b));
			return strcmp($an, $bn);
		});
